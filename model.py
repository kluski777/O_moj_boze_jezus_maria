...
class ActorCritic(nn.Module, AbstractCar):
    def __init__(self, name, feature_dim, N=2048, batch_size=32, K=8, lr=1e-3, gamma=0.99, lam=0.95, eps=0.1, entropy_coef=0.05):
        ...
        self.backbone = nn.Sequential(
            ...
            nn.GroupNorm(1, 32),  # GroupNorm(1, C) == LayerNorm over (C,H,W), robust to spatial size
            nn.GELU(),
        ...
        # to materialize LazyLinear then apply orthogonal init
        with torch.no_grad():
    ...
    def _init_weights(self):
        ...
        nn.init.orthogonal_(self.actor[-1].weight,  gain=0.01)  # low gain → high entropy at start
    ...
    def update(self, last_next_state):
        ...
        ev = (1.0 - (returns - values.detach()).var() / (returns.var() + 1e-8)).item()

        value_std  = values.detach().std().item()
        ...
        total_ratio  = 0.0
        total_value  = 0.0
        layer_norm_accum = {}  # accumulates per-module update-to-weight ratio
        count        = 0
        ...
        for _ in range(self.K):
            ...
            for start in range(0, self.N, self.batch_size):
                ...
                critic_loss  = F.mse_loss(mb_values, returns[mb])  # MSE: error-proportional grads escape the collapse-to-mean plateau
                ...
                # snapshot weights before the step so we can measure the actual update
                ...
                # Karpathy update-to-weight ratio per module: |Δw| / |w|  (Adam-aware, scale-invariant)
                upd_sq = {}
                wgt_sq = {}
