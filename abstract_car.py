...
class AbstractCar:
    def __init__(self, name):
        ...
        self.mask = None
        self.name = name
        self.checkpoint_distance = 0.0 # Distance from the next checkpoint
    ...
    def update_progress(self, checkpoints):
        ...
        if self.checkpoint_index < len(checkpoints):
            ...
            self.x_diff = checkpoint_x - self.x
            self.y_diff = checkpoint_y - self.y

            distance = math.sqrt(self.x_diff ** 2 + self.y_diff ** 2)
            self.checkpoint_distance = distance

            self.angle_to_checkpoint = math.degrees(math.atan2(-self.x_diff, -self.y_diff)) % 360
